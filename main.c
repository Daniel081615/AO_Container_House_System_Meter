...
uint8_t fgToReaderFlag;		// ¶Ç°e¦ÜReader
uint8_t fgToReaderRSPFlag;	// ¦^À³ Reader ³q°TºX¼Ð
uint8_t fgToHostFlag;		// ¶Ç°e¦ÜHost
uint8_t fgToHostRSPFlag;		// ¦^À³ Host ³q°TºX¼Ð
uint8_t fgReaderFunction;	// polling°Æµ{¦¡¨Ï¥Î (¥\¯àºX¼Ð )
...
uint16_t	ConsumptionCounter;			// ¹q¤O­p¼Æ 1600 = 1«× (1KW)
...
uint8_t RoomSetting;
uint8_t AutoLockTime;			// ¦Û°Ê¤WÂêµ¥«Ý®É¶¡
...
#define TEST_EE_ADDR 0x0000
 ...
 // ¶}¾÷µ¥«Ý 2.5 Sec Åª¨ú¹q¿ö­È¤Î¦P¨BCenter®É¶¡
...
MeterTxBuffer[0]= 0x01;	//	DeviceID
CmdModbus_BMS(MDBS_SET_BMS_DEVICE_ADDR);

MeterTxBuffer[0]= 0x01;
ModeFlags |= 1 << 3;			// Set RS-485
CmdModbus_BMS(MDBS_SET_MODES);

/***	@WaterMeter test & Setup function  ***/
UART2_ChangeBaudrate(9600);
WMSetDeviceID = 0x01;			// Set DeviceID
WMBaudRate = 0x03;				// Set baudrate to 2400	
