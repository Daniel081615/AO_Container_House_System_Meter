		...
		switch(SoilSensorPollingState)	
		{
				case	SS_POLLING_READY:
						if (TickReadPowerTime >= ReadMeterTime)
						{
								...
								if (SoilSensorPollingState > SS_POLLING_MOISTURE)
								{
										PollingSoilSensorID++;
								...
								switch ( SoilSensorModbusCmd )	
								{
										...
										case MBSSCMD_GET_DEVICE_ADDRESS:
												SoilSensorPollingState = SS_GET_DEVICE_ADDRESS;
												SoilSensorModbusCmd = MBSSCMD_READY;
												break;
										...
										case MBSSCMD_GET_BAUD_RATE:
												SoilSensorPollingState = SS_GET_BAUD_RATE;
												SoilSensorModbusCmd = MBSSCMD_READY;
												break;
				...
				case	SS_POLLING_RSP:
            if (GotDeviceRsp == PollingSoilSensorID)
            {
                SoilSensorSuccess();
            } else {
...
void SoilSensorDataProcess(void)
{
		...
		uint16_t u16tmp;
		uint32_t u32tmp;
		SoilSensorArrayIndex = GotDeviceRsp -1;
		
	
		if (GotDeviceRsp != PollingSoilSensorID) return;	//	check the host address idx
        ...
        case MDBS_GET_COEFS:
            SoilSensorData[SoilSensorArrayIndex].EC_Temp_Coef 	= ((TokenMeter[3] << 8) | TokenMeter[4]);
...
void MODBUS_SendSoilSensorCmd(uint8_t cmd)
{
		MeterTxBuffer[0] = PollingSoilSensorID ; 
...
void CmdModbus_SoilSensor_SN3002TR(uint8_t ModbusCmd)
{
		...
		{				
				...
				case MDBS_SET_SS_BAUD_RATE:
						...
						MeterTxBuffer[4] = (SSNewBaudRate >> 8) & 0xff; 
						MeterTxBuffer[5] = SSNewBaudRate & 0xff; 
						CRC16(MeterTxBuffer, 6);
						MeterTxBuffer[6] = uchCRCHi;
						MeterTxBuffer[7] = uchCRCLo;
						_SendStringToMETER(MeterTxBuffer, 8);
...
void SoilSensorSuccess(void)
{
		...
		SoilSensorError.SSDeviceNG &= (~(0x00000001 << (PollingSoilSensorID -1)));
