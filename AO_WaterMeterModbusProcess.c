...
WtrMtrError_t WtrMtrError;
WMData_t WMData[WtrMtrMax];
uint8_t PollingWtrMtrID, WMSetDeviceID;
...
void WMPolling(void)
{
		...
		{	
						...
						if (TickReadPowerTime >= ReadMeterTime)
            {
								...
								WtrMtrModbusCmd =  WtrMtrCmdList[PollingWtrMtrID-1];
								WtrMtrCmdList[PollingWtrMtrID-1] = MBWMCMD_READY ;
...
void MODBUS_SendWMCmd(uint8_t cmd)
{
		MeterTxBuffer[0] = WtrMtrIDArray[PollingWtrMtrID-1]; 
