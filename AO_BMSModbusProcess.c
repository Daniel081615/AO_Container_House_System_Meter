...
uint8_t PollingBmsID, BmsModbusCmd;
uint8_t BmsPollingState;
uint8_t GotDeviceRsp;
uint8_t BmsPollingStateIndex;
uint8_t BmsReadErrorCnt;

uint16_t BmsMBCmd, ModeFlags;
uint32_t ChargeOnOff, DischargeOnOff, BatteryBalanceOnOff, BmsNewAddr, CellCount;
		...
		{	
				...
				case	BMS_POLLING_READY:
						...
						if (TickReadPowerTime >= ReadMeterTime)
						{
								...
								//	
								BmsModbusCmd = BmsCmdList[PollingPwrMtrID-1] ;
								BmsCmdList[PollingPwrMtrID-1] = MBBMSCMD_READY;
				...
				case	BMS_POLLING_RSP:
						if (GotDeviceRsp == BmsIDArray[PollingBmsID-1])
            {
                BmsSucccess();
            } else {
                BmsTimeoutProcess();
...
{
		//	BMS ID
		MeterTxBuffer[0] = BmsIDArray[PollingBmsID-1]; 
		GotDeviceRsp = 0xFF ;	
		//	Send BMS Cmd
		CmdModbus_BMS(cmd);
}
...
void CmdModbus_BMS(uint8_t ModbusCmd)
{
		...
		{	
				...
				case	MDBS_SET_BMS_DEVICE_ADDR:
						...
						MeterTxBuffer[7] = (BmsNewAddr >> 24) & 0xFF;
						MeterTxBuffer[8] = (BmsNewAddr >> 16) & 0xFF;
						MeterTxBuffer[9] = (BmsNewAddr >> 8)  & 0xFF;	
						MeterTxBuffer[10] =(BmsNewAddr)			  & 0xFF;
				...
				case	MDBS_SET_BATTERY_CHARGEING_STATUS:
						...
						MeterTxBuffer[5] = 0x02;
						MeterTxBuffer[6] = 0x04;
						MeterTxBuffer[7] = (ChargeOnOff >> 24) & 0xFF;
						MeterTxBuffer[8] = (ChargeOnOff >> 16) & 0xFF;
						MeterTxBuffer[9] = (ChargeOnOff >> 8)  & 0xFF;
						MeterTxBuffer[10] =(ChargeOnOff)			 & 0xFF;		// 00: off / 01: on
						CRC16(MeterTxBuffer, 11);
						MeterTxBuffer[11]=uchCRCHi;
						MeterTxBuffer[12]=uchCRCLo;
						_SendStringToMETER(MeterTxBuffer,13);   			
				...
				case	MDBS_SET_BATTERY_DISCHARGING_STATUS:
						...
						MeterTxBuffer[5] = 0x02;
						MeterTxBuffer[6] = 0x04;
						MeterTxBuffer[7] = (DischargeOnOff >> 24) & 0xFF;
						MeterTxBuffer[8] = (DischargeOnOff >> 16) & 0xFF;
						MeterTxBuffer[9] = (DischargeOnOff >> 8) & 0xFF;
				...
				case	MDBS_SET_BATTERY_BALANCE_STATUS:
						...
						MeterTxBuffer[8] = (DischargeOnOff >> 16) & 0xFF;
						MeterTxBuffer[9] = (DischargeOnOff >> 8) & 0xFF;
						MeterTxBuffer[10] = BatteryBalanceOnOff & 0xFF;	// 00: off / 01: on
						CRC16(MeterTxBuffer, 11);
						MeterTxBuffer[11]=uchCRCHi;
						MeterTxBuffer[12]=uchCRCLo;
						_SendStringToMETER(MeterTxBuffer,13);   			
				...
				case	MDBS_SET_MODES:
						...
						MeterTxBuffer[7] = (ModeFlags >> 8) &0xFF;
						MeterTxBuffer[8] = ModeFlags & 0xFF;
...
void BmsSucccess(void)
{
		...
		BmsError.BmsDeviceNG &= (~(0x00000001 << (BmsArrayIndex)));
